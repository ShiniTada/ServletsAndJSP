...
<head>
    ...
    <fmt:message bundle="${loc}" key="local.admin.orders_table.first" var="Id"/>
    <fmt:message bundle="${loc}" key="local.admin.orders_table.second" var="Customer"/>
    <fmt:message bundle="${loc}" key="local.admin.orders_table.third" var="Courier"/>
    <fmt:message bundle="${loc}" key="local.admin.orders_table.forth" var="From"/>
    <fmt:message bundle="${loc}" key="local.admin.orders_table.fifth" var="To"/>
    <fmt:message bundle="${loc}" key="local.admin.orders_table.sixth" var="Date"/>
    <fmt:message bundle="${loc}" key="local.admin.orders_table.seventh" var="Status"/>
    ...
    <div>
        ...
        <div class="w3-sidebar w3-light-grey w3-bar-block" style="width:15%">
            ...
            <div class="sign-in">
                <form action="Controller" method="get">
                    ...
                    <button class="w3-bar-item w3-button btn-lg btn-block w3-teal w3-padding-large">${orders}</button>
                </form>
            </div>
        ...
        <div style="margin-left:20%; margin-right:15%;">
            <div class="w3-container w3-center">
                <h2 class="title w3-text-teal" style="margin-top:100px;">${orders}</h2>
                <table class="w3-table-all">
                    ...
                    <c:choose>
                        <c:when test="${requestScope.pagination eq 2}">
                            <c:forEach var="elem" items="${sessionScope.listOrders}" begin="5" end="9">
                        ...
                        <c:when test="${requestScope.pagination eq 3}">
                            <c:forEach var="elem" items="${sessionScope.listOrders}" begin="10" end="14">
                                <tr class="w3-hover-light-blue">
                                    <td><c:out value="${elem.id}"/> </td>
                                    <td><c:out value="${elem.customer.login}"/> </td>
                                    <td><c:out value="${elem.courier.login}"/> </td>
                        ...
                        <c:when test="${requestScope.pagination eq 4}">
                            <c:forEach var="elem" items="${sessionScope.listOrders}" begin="15" end="19">
                                <tr class="w3-hover-light-blue">
                                    <td><c:out value="${elem.id}"/> </td>
                                    <td><c:out value="${elem.customer.login}"/> </td>
                                    <td><c:out value="${elem.courier.login}"/> </td>
                        ...
                        <c:otherwise>
                            <c:forEach var="elem" items="${sessionScope.listOrders}" begin="0" end="4">
                                <tr class="w3-hover-light-blue">
                                    <td><c:out value="${elem.id}"/> </td>
                                    <td><c:out value="${elem.customer.login}"/> </td>
                                    <td><c:out value="${elem.courier.login}"/> </td>
