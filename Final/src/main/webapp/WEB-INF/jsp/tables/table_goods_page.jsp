...
<div>
    ...
    <div style="margin-left:20%; margin-right:15%;">
        <div class="w3-container w3-center">
            <h2 class="title w3-text-black" style="margin-top:100px;">${goods}</h2>
                    ...
                    <th>${Courier}</th>
                        ...
                        <c:forEach var="elem" items="${sessionScope.listGoods}">
                            <tr class="w3-hover-light-blue">
                                <c:choose>
                                <c:when test="${elem.typeGoods.getValue() eq 'food'}"><td><c:out value="${food}"/> </td></c:when>
                                <c:when test="${elem.typeGoods.getValue() eq 'tech'}"><td><c:out value="${tech}"/> </td></c:when>
                                <c:when test="${elem.typeGoods.getValue() eq 'furniture'}"><td><c:out value="${furniture}"/> </td></c:when>
                                    <c:when test="${elem.typeGoods.getValue() eq 'easy-to-beat'}"><td><c:out value="${easy_to_beat}"/> </td></c:when>
                                </c:choose>
            ...
            <div class="pagination">
                <my:paginator limit="${sessionScope.limitCount}" total="${sessionScope.totalCount}" command="get-all-goods"/>
            </div>
        </div>
